"""Orquestrador do pipeline de ciência de dados — executa as etapas em sequência."""
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(description="Pipeline de predicao de chuva — Rain in Australia")
    parser.add_argument("--skip-eda", action="store_true", help="Pula a EDA (carrega processed/ se existir, senao gera sem figuras).")
    parser.add_argument("--skip-tuning", action="store_true", help="Pula o tuning; treina modelos com parametros padrao.")
    return parser.parse_args()


def _banner(titulo: str) -> None:
    """Imprime um banner pra a seção."""
    sep = "=" * 65
    print(f"\n{sep}")
    print(f"  {titulo}")
    print(f"{sep}")
...
def run_pipeline(skip_eda: bool = False, skip_tuning: bool = False) -> None:
    ...
    _banner("ETAPA 5c — Geração de Figuras (Fig 8–12)")
    import matplotlib
    matplotlib.use("Agg")
    ...
    print("[main] Fig 8 — Matriz de confusão...")
    plot_confusion_matrix(best_name, y_test, best_result["y_pred"])
    plt.close("all")

    print("[main] Fig 9 — Curvas ROC...")
    plot_roc_curves(results, y_test)
    plt.close("all")

    if best_result["y_prob"] is not None:
        print("[main] Fig 10 — Curva Precision-Recall...")
        plot_pr_curve(best_name, y_test, best_result["y_prob"])
        plt.close("all")

    print("[main] Fig 11 — Comparativo de métricas...")
    plot_metrics_comparison(df_metrics)
    plt.close("all")

    print("[main] Fig 12 — Importância das variáveis...")
